...
public class MockUtilTest {
  ...
  public void testAndMatch() {
    ...
    // 模拟输入的 mockReqParams
    String mockReqParams =
        "[{\"jsonPath\":\"$.key1\",\"expectedValue\":\"value1\"}, {\"jsonPath\":\"$.key2\",\"expectedValue\":\"value2\"}]";

    // 模拟解析 mockReqParams 返回的 mockExpressions

    boolean andMatch =
        MockUtil.andMatch(
            jsonObject, JSONArray.parseArray(mockReqParams, MockExpression.class), new HashMap<>());
  ...
  public void testGetMockData() {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      mockData.setMockExpressions(
          JSONArray.parseArray(mockData.getMockReqParams(), MockExpression.class));
